...
def run():
    ...
    logger = utils.setup_logger("BlackFriday", log_file=os.path.join(paths.LOG_PATH, "black_friday.log"))
    logger.info("Black Friday modul indul.")

    bf_basic_path = os.path.join(paths.BLACK_FRIDAY_PATH,"BasicTable.xlsx")
    sheets = pd.ExcelFile(bf_basic_path)
...
# teljes SQL az eredetiből
    ...
    for sn in sheets.sheet_names:
        if isinstance(sn, str) and sn.startswith('phase'):
            try:
              t = pd.read_excel(bf_basic_path, sheet_name=sn)
            except Exception as e:
                logger.error(f"Hiba történt a {bf_basic_path} olvasása közben: {e}")
                continue
            t = utils.excel_tpn_integer_konverzio(bf_basic_path, sheet_name=sn, konvertalas=False)
    ...
    if base is None or (hasattr(base, 'empty') and base.empty):
        ...
        database.DatabaseManager.smtp_email_send(subject=f"[ERROR] : Black friday report",
                                    sender_mail="Patrik.Major@tesco.com",
                                    receiver_mail="Patrik.Major@tesco.com",
                                    cc_mail=None,
                                    html=f"The Black friday background table is not ready to use! error: {final_err_msg}",
                                    bcc_mail=None)
    else:
        ...
        output_path = os.path.join(paths.BLACK_FRIDAY_PATH,"black_friday.csv")
        base.to_csv(output_path, index=False)
        logger.info(f"Black Friday CSV mentve: {output_path}")

        max_day = str(base.Date.max())
        day, month, year = max_day[-2:], max_day[4:-2], max_day[:4]
        logger.info(f"Black Friday tracker up to date: {day}-{month}-{year}")
        database.DatabaseManager.smtp_email_send(subject=f"Black Friday is up todate {year}-{month}-{day}",
                                                sender_mail="patrik.major@tesco.com",
                                                ...
                                                bcc_mail=None)
        logger.info("Black Friday modul kész. Email elküldve.")
